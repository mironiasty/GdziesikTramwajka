...
export async function searchForStops(partialName) {
  ...
  const response = await fetch(
    `http://www.ttss.krakow.pl/internetservice/services/lookup/autocomplete/json?query=${partialName}`
  );
...
export async function getTripInfo(tripId) {
  console.log(tripId);
  const response = await fetch(
    `http://www.ttss.krakow.pl/internetservice/services/tripInfo/tripPassages?tripId=${tripId}`
  );
